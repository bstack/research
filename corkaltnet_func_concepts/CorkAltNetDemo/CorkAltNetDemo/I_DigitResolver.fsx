...
let digitDataSample = getRecordsFromFile @"D:\Temp\research\ktug_think_functionally\Library1\trainingsample.csv"
let digitValidationSample = getRecordsFromFile @"D:\Temp\research\ktug_think_functionally\Library1\validationsample.csv"
...
let classify (unknown:int[]) =
    let newMap = digitDataSample |> Array.map (fun (record) -> record, calculateDistance unknown record.Pixels)
    let record, distance = newMap |> Array.minBy (fun (record, distance) -> distance)
    record.Answer

let evaluateClassifier = 
    let res = 
        digitValidationSample |>
